...
author: Di Cook
date: '2018-08-28'
slug: content/post/2018-08-28-tourr-with-plotly-in-xaringan
categories: [data visualisation,slides]
tags: [animation,tour,R,tidyverse,xaringan,slides]
header:
  caption: ''
  image: ''
---
...
This can now be played using `ggplotly`, after first plotting using `ggplot2`. *Note the `aes(frame = indx)` which throws a warning from `ggplot`. It is essential for making the animation.*
...
The plotly animation, although it runs nicely in the RStudio viewer, and in this blog post, does not automatically run in the html slides. You need to save it to html, using the `htmltools` function:
