...
categories: [statistical graphics,data visualisation,R,tidyverse,ggplot2,time series,smart meter]
tags: [statistical graphics,data visualisation,R,tidyverse,ggplot2,time series,smart meter]
header:
  caption: ''
  image: ''
---
...
ggplot(elec, aes(x=days, y=kwh, colour=meter)) + geom_line() + scale_colour_brewer(palette = "Dark2")
ggplot(elec, aes(x=halfhour, y=kwh, colour=meter, group=date)) + geom_line() +
  facet_grid(month~wday) + scale_colour_brewer(palette = "Dark2")
d <- elec %>% filter(date == ymd("20180321"))
ggplot(d, aes(x=halfhour, y=kwh)) + geom_line() + facet_wrap(~meter)
```
...
```{r eval = FALSE}
march <- elec_hr %>% filter(month == "Mar")
ggplot(march, aes(x=hour, y=kwh, group = date, colour = meter)) +
  geom_line() + 
  facet_grid(meter~work) + 
  scale_colour_brewer(palette = "Dark2")
ggplot(march, aes(x=hour, y=kwh, colour = meter)) +
  geom_point(alpha=0.5) + 
  geom_smooth(se=FALSE) + 
  facet_grid(meter~work) + 
  scale_colour_brewer(palette = "Dark2")
ggplot(elec_hr, aes(x=hour, y=kwh, colour = meter)) +
  geom_line(aes(group=date), alpha=0.2) + 
  geom_smooth(se=FALSE, size=1) + 
  facet_grid(meter+month~work) + 
  scale_colour_brewer(palette = "Dark2")
```

## Sources

[Code is here.](https://github.com/dicook/dicook.github.io/blob/blogdown/content/post/2018-04-22-smart-meters.Rmd)
